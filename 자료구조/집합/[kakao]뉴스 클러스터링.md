...
- 풀긴 풀었으나 여러 자료구조를 사용하여 공간 복잡도도 커지고 깔끔하지 않음

```python
def solution(str1, str2):
    answer = 0
    inter_set, sum_set = [], []
    dict1, dict2 = make_dict(str1), make_dict(str2)
    for key in dict1.keys():
        if key in dict2:
            for _ in range(min(dict1[key], dict2[key])):
                inter_set.append(key)
            for _ in range(max(dict1[key], dict2[key])):
                sum_set.append(key)
        else:
    ...
    for key in dict2.keys():
        if key not in dict1:
            for _ in range(dict2[key]):
                sum_set.append(key)

    if len(sum_set) == 0: return 65536
    return int(len(inter_set) / len(sum_set) * 65536)
