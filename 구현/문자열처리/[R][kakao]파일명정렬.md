...
- 런타임 에러가 난다면 왜 나는지 테스트 케이스를 잘 고민해보기

```python
from functools import cmp_to_key

def solution(files):
    d_files = divide_files(files)
    d_files.sort(key=cmp_to_key(comperator))

    return ["".join(x) for x in d_files]
...
def divide_files(files):
    ...
    for file in files:
        ...
        for i in range(len(file)):
            if not flip and 48 <= ord(file[i]) <= 57:
                divided.append(file[:i])
                flip, idx1 = True, i
            elif flip:
                if (ord(file[i]) > 57 or ord(file[i]) < 48) or i == idx1 + 5:
                    divided.append(file[idx1:i])
                    divided.append(file[i:])
                    break
        else:
            divided.append(file[idx1:])
        _files.append(divided)
    return _files

def comperator(a, b):
    head_a, head_b = a[0].upper(), b[0].upper()
    if head_a > head_b:
        return 1
    elif head_a < head_b:
        return -1
